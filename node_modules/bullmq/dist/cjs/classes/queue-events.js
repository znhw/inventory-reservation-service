...
class QueueEvents extends queue_base_1.QueueBase {
    ...
    async run() {
        if (!this.running) {
            try {
                ...
                try {
                    await client.clientSetName(this.clientName(utils_1.QUEUE_EVENT_SUFFIX));
                }
                catch (err) {
    ...
    async consumeEvents(client) {
        ...
        while (!this.closing) {
            ...
            const data = await this.checkConnectionError(() => client.xread([{ key, id }], { BLOCK: opts.blockingTimeout }));
            this.blocking = false;
            if (data) {
                const stream = data[0];
